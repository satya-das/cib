...
struct __zz_cib_Helper<::ExampleClass, T> : public __zz_cib_MethodTableHelper {
  ...
  static auto get_3(__zz_cib_AbiType __zz_cib_obj) {
  ...
  template <typename _RT>
  static auto value_4(const __zz_cib_AbiType __zz_cib_obj) {
    using __zz_cib_ProcType = _RT (__zz_cib_decl *) (const __zz_cib_AbiType);
    return __zz_cib_GetMethodTable().Invoke<__zz_cib_ProcType, __zz_cib_MethodId::value_4>(
      __zz_cib_obj
      );
  }
...
template <typename T>
bool __zz_cib_Helper<::ExampleClass, T>::instanceDeleted_ = false;
}
