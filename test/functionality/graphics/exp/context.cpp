...
namespace __zz_cib_ { namespace Graphics { namespace Context { namespace __zz_cib_Generic {
	...
	public:
		...
		void Line(float x1, float y1, float x2, float y2) override {
			...
			auto method = __zz_cib_get_mtable_helper().getMethod<LineProc>(__zz_cib_methodid::Line_2);
		...
		void Rect(float l, float b, float r, float t) override {
			...
			auto method = __zz_cib_get_mtable_helper().getMethod<RectProc>(__zz_cib_methodid::Rect_3);
		...
		void Circle(float cx, float cy, float r) override {
			...
			auto method = __zz_cib_get_mtable_helper().getMethod<CircleProc>(__zz_cib_methodid::Circle_4);
		...
		void Ellipse(float cx, float cy, float a, float b) override {
			...
			auto method = __zz_cib_get_mtable_helper().getMethod<EllipseProc>(__zz_cib_methodid::Ellipse_5);
			return method(__zz_cib_h_, cx, cy, a, b);
		}
		~Context() override {
			if (!__zz_cib_h_) return;
			using __zz_cib_deleteProc = void (__zz_cib_decl *) (__zz_cib_HANDLE*);
			auto method = __zz_cib_get_mtable_helper().getMethod<__zz_cib_deleteProc>(__zz_cib_methodid::__zz_cib_delete_6);
			method(__zz_cib_h_);
		}
	};
}}}}
