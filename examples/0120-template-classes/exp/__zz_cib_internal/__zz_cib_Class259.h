...
template <typename T>
struct __zz_cib_Helper<::Example::Value<float>, T> : public __zz_cib_MethodTableHelper {
  static_assert(std::is_same_v<T, ::Example::Value<float>>,
...
template <typename T>
bool __zz_cib_Helper<::Example::Value<float>, T>::instanceDeleted_ = false;
}
namespace Example {
template<>
class Value<float>
...
public:
  ...
  float GetValue() const {
    return __zz_cib_::__zz_cib_FromAbiType<float>(
      __zz_cib_MyHelper::GetValue_3<__zz_cib_::__zz_cib_LazyAbiType_t<__zz_cib_Dummy, float>>(
  ...
  template <typename __zz_cib_Dummy = std::pair<Value<float>*, Value<float>*>>
  bool operator ==(const ::Example::Value<float>& rhs) const {
...
private:
  __ZZ_CIB_TEMPLATE_CLASS_INTERNALS(__ZZ_CIB_CLASS_NAME(Value<float>), __ZZ_CIB_CLASS_NAME(Example::Value<float>));
};
}
