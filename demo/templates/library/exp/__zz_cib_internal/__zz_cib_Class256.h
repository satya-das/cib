...
struct __zz_cib_Helper<::Value<int>, _T> : public __zz_cib_MethodTableHelper {
  static_assert(std::is_same_v<_T, ::Value<int>>);
...
public:
  ...
  Value(int value)
    ...
    {}
  template <typename _ThisClass = Value<int>>
  int get() const {
    using __zz_cib_T  = std::pair<_ThisClass, Value<int>>;
